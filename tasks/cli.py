...
def cli(ctx, service, clean=False):
    ...
    if clean:
        ...
        docker_cmd = "docker run -i -d --name {} {}:{}".format(tmp_ctr_name, EXAMPLES_BUILD_IMAGE_NAME, get_version())
        run(docker_cmd, shell=True, check=True)
        run("docker cp examples-build:/usr/local/faasm/wasm ./dev/faasm-local/wasm/", shell=True, check=True)
        # run("docker cp examples-build:/usr/local/faasm/shared ./dev/faasm-local/shared/", shell=True, check=True)
